...
public class PostsApiController {
    ...
    @GetMapping("/update")
    public ResponseEntity<PostsResponseDto> update(@RequestParam("category") String category,
    ...
    // 그 내용을 db에 저장함
    @PutMapping("/update")
    public ResponseEntity<?> update(@RequestParam("category") String category, @RequestParam("id") Long id,
    ...
    public ResponseEntity<PostsAndCommentsResponseDto> postview(@RequestParam("category") String category,
            @RequestParam("id") Long id) {
        return ResponseEntity.ok()
    ...
    // 댓글 페이징 조회
    @GetMapping("/view")
    public ResponseEntity<?> postview(Model model, @RequestParam("category") String category,
            @RequestParam("id") Long id,
            @RequestParam(required = false, defaultValue = "0", value = "commentPage") int page,
            /* @LoginUser SessionUser user */ @RequestParam("email") String email) {
    ...
    /* 게시판 목록 조회 */
    @GetMapping("/list")
    public ResponseEntity<?> list(Model model, @RequestParam("category") String category,

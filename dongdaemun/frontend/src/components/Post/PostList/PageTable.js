...
import {BrowserRouter as BrowserRouter, Routes, Route, Link, useParams} from "react-router-dom"
import pagination from './Pagination';
...
const renderPosts = posts.posts && posts.posts.map(post => {
  ...
  return (
      <div  key={post.id}>
        <a href= {`readpost/${category}/${post.id}`} style={{textDecoration: 'none'}} className='posts'>
  ...
  return (
    ...
    <div className="tablecss">
      ...
      <a href ={`makepost/${category}`} style={{textDecoration: 'none'}}>
        <button className="write" >글 작성하기</button></a>
      </div>
